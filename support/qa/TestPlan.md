...
1. Attempt to start a second copy of Meazure. Verify that the first instance is restored to its original position
...
1. Exit Meazure
1. Install the latest version of Meazure accepting all default settings
...
1. After installation is complete verify the following:
   ...
   a. Start Meazure and verify that the help menu is disabled. Exit Meazure
...
1. Verify that hovering over the Point 1 crosshair shows the tool tipe `Point 1`. Repeat for Point 2.
...
1. Verify the `As` field is disabled
1. Move the mouse over each crosshair. Verify the crosshair color changes to yellow when cursor enters 
...
1. Verify that the angle display reads 1.57 (i.e. pi / 2)
...
1. In the `Names` field enter `Miles`
...
1. On the `Edit` menu select `Preferences`, then select the `Custom Units` tab
...
1. On the `Edit` menu select `Preferences`, then select the `Custom Units` tab
1. Press the `Set Display Precision` button and verify that the Precision panel is then selected 
   with the custom units radio button selected
...
1. Verify that the crosshair should not be visible in the magnifier window.
1. On the toolbar select the Cursor Tool
1. Verify that a red square is displayed at the center of the magnifier window
1. Move the Zoom slider to `6X` and verify that a grid is now displayed in the magnifier window
...
1. On the `View` menu select `Color Format` and `Hexadecimal`
...
1. Start the program and dismiss the splash screen
...
1. Select the Cursor Tool
1. Exit the program
1. Double click the profile file and verify that Meazure is launched with the Point Tool and 
   Ruler Tool selected
1. Turn off the Rulers
1. Select the Cursor Tool
1. Exit the program
1. Drag the profile file and drop it on the Meazure program icon. Verify that Meazure is 
   launched with the Point Tool and Ruler Tool selected
1. On the `Edit` menu select `Preferences`
...
1. Delete the file `C:\Documents and Settings\baron\Application Data\Microsoft\HTML Help\hh.dat`
1. Start the program
1. On the `Help` menu select the `Contents` item
1. Verify that the online help window appears with the `Context` tab selected
1. On the `Help` menu select the `Search` item
1. Verify that the online help window appears with the `Search` tab selected
1. On the `Help` menu select the `Index` item
1. Verify that the online help window appears with the `Index` tab selected
1. On the `Help` menu select the `What’s This` item.
1. Click on the Tool Information section and verify that the help for the Tool Information section is displayed
...
1. Click on the URL and verify that the C Thing Software home page is displayed in a web browser
